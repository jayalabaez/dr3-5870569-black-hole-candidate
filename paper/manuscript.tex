...
%  MNRAS format — enhanced publication version
% ====================================================================
...
\label{caveat}
This source carries an \texttt{AstroSpectroSB1} solution ---
the strongest solution type in the Gaia NSS catalogue ---
combining independent astrometric and spectroscopic orbital
constraints.  This dual confirmation substantially reduces
the probability of a spurious solution compared to purely
astrometric \texttt{Orbital} solutions.  Nevertheless, the
companion mass and all derived properties remain
\emph{conditional on the Gaia pipeline model being a correct
description of this system}, including the adopted primary
mass, the validity of the single-Keplerian fit, and the
parallax.  Independent ground-based multi-epoch RV monitoring
would provide definitive dynamical confirmation.}}
\medskip
...
$T_{\rm eff} = 4832\,$K and $\log g = 3.255$.  The grid returns
$M_1 \approx 0.67$--$1.83\,\Msun$, with higher masses at higher
...
yields a median $M_1 = 0.90\,\Msun$ with 68~per~cent CI
$[0.66,\,1.30]\,\Msun$ and 90~per~cent CI
$[0.53,\,1.69]\,\Msun$.  The adopted
$M_1 = 1.04 \pm 0.20\,\Msun$ falls within the central
...
(script~12).  The resulting \emph{synthetic} companion-to-primary
flux ratios for a $12.55\,\Msun$ MS companion are
$\sim$\,$8$--$33\times$ across optical--infrared bands
(photometric radius) --- all far exceeding the 5~per~cent
detection threshold.  The Planck and synthetic ratios agree to
within 1--3~per~cent across all bands, confirming that the
simple Planck model is adequate for this exclusion.

The maximum main-sequence companion that remains hidden below a
5~per~cent threshold is $M \lesssim 1.1\,\Msun$
(photometric $R$) or $\sim 0.8\,\Msun$ ($\log g$-based $R$)
--- far below the derived $M_2$.

We also scan stripped helium star models
\citep{Gotberg2018} at masses of $1$--$15\,\Msun$.  Even the
lowest-mass He star model ($1.0\,\Msun$) would produce a
G-band flux ratio of $\sim 0.84\times$ the primary ---
easily detectable.  At the catalogue mass ($12.55\,\Msun$),
the He star would outshine the primary by $\sim 8\times$ in
$G$.  Stripped He companions are therefore firmly excluded.
...
\subsection{$T_{\rm eff}$ chain dependency}
\label{sec:methods:teff_chain}
...
the companion mass are all affected downstream.  To quantify the
impact, we propagate $T_{\rm eff}$ offsets of
$\pm 100$, $\pm 250$, and $\pm 500\,$K through the full chain
$T_{\rm eff} \to L_\star \to R_\star \to M_1 \to M_2$
(script~14).

At $T_{\rm eff} = 4332\,$K ($-500$\,K), the fiducial $M_2$
increases to $\sim 14.8\,\Msun$; at
$T_{\rm eff} = 5332\,$K ($+500$\,K), $M_2$ decreases to
$\sim 8.8\,\Msun$.  Parallax perturbation (±$1\sigma$) shifts
$M_2$ from $\sim 7.7$ to $\sim 17.1\,\Msun$, confirming that
parallax dominates the error budget.  The sensitivity
${\rm d}M_2/{\rm d}T_{\rm eff} \approx -0.006\,\Msun\,{\rm K}^{-1}$
is small enough that the BH classification remains secure
across the full $\pm 500\,$K range.  In all cases the companion
mass exceeds $5\,\Msun$.
...
\texttt{AstroSpectroSB1} solutions, the Monte~Carlo draws in
...
repeat the $5\times10^5$-draw experiment under several plausible
inter-parameter correlation structures (script~11).

\textbf{Parallax--angular-semi-major-axis correlation.}  We
inject bivariate-normal correlations with
$\rho = 0,\,0.3,\,0.5,\,0.7$ between $\varpi$ and the angular
semi-major axis.  The resulting $P(M_2 > 5\,\Msun)$
\emph{increases} monotonically:
88~per~cent ($\rho = 0$), 91~per~cent ($\rho = 0.3$),
94~per~cent ($\rho = 0.5$), 96~per~cent ($\rho = 0.7$).
The direction is expected: both quantities share a common
astrometric signal, so a positive correlation partially cancels
when they enter the mass equation, narrowing the posterior.
Independent sampling is therefore \emph{conservative} for this
parameter pair.
...
the fractional period uncertainty ($\sigma_P/P \approx 3.4$~per~cent)
contributes only a small fraction of the mass variance relative to
the parallax.

\textbf{Systematic-bracket expansion.}  Expanding the uniform
$\pm 5$~per~cent multiplicative systematic to $\pm 10$~per~cent and
$\pm 15$~per~cent changes $P(M_2 > 5\,\Msun)$ by $< 0.3$~percentage
points, and the median $M_2$ remains at $12.5\,\Msun$.
...
\item \textbf{Directly measured.}  Observed multi-band photometry
  ($G$, $G_{\rm BP}$, $G_{\rm RP}$),
  parallax ($\varpi = 0.672 \pm 0.104$\,mas), systemic radial
  velocity ($v_{\rm sys} = -5.04 \pm 2.08\,\kms$), and
  ROSAT/XMM/eROSITA/RACS/SUMSS non-detections.  These are empirical
...
\item \textbf{Pipeline-derived (model-dependent).}  The Gaia NSS
  ...
  axis, inclination) from the \texttt{AstroSpectroSB1} pipeline
  fit combining both along-scan astrometry and RVS radial velocities.
  The GSP-Phot stellar parameters
  ($T_{\rm eff}$, $\log g$), which are known to be unreliable at
  RUWE~$= 9.22$ \citep{Andrae2023}.

\item \textbf{Assumed.}  The primary mass
  ($M_1 = 1.04 \pm 0.20\,\Msun$) from isochrone matching of
  GSP-Phot parameters (cross-checked against PARSEC grid;
  Section~\ref{sec:methods:m1}).  The $\times 1.5$ parallax-error
  inflation factor.  The 10~per~cent Gaussian systematic.

\item \textbf{Conditional inference.}  The companion mass
  posterior ($M_2 = 12.5\,\Msun$), the dark-companion
  classification, and all alternative-scenario exclusions.  These
  are valid \emph{only if} the pipeline solution and adopted
  stellar model are approximately correct.
\end{enumerate}

\noindent
The \texttt{AstroSpectroSB1} solution type places items in Tier~2
on substantially firmer ground than for pure \texttt{Orbital}
solutions, because the orbital signal is independently confirmed
by radial velocities.  This is a critical distinction:
population-level validation studies find that $> 30$~per~cent of
pure \texttt{Orbital} solutions fail RV checks
\citep{ElBadry2024}, but this failure rate is much lower for
\texttt{AstroSpectroSB1} solutions where the RV signal is
already built into the fit.
...
\subsection{Context among AstroSpectroSB1 BH candidates}
\label{sec:discussion:population}

The GRAVITAS OmniScan v13 catalogue contains 14
\texttt{AstroSpectroSB1} solutions with pipeline-reported
$M_2 > 5\,\Msun$ (i.e.\ BH candidates).  For each we compute
the predicted edge-on RV semi-amplitude $K_{1,\rm edge}$
(equation~\ref{eq:K1}) as a standardised ranking metric.
This source ranks 3rd among the 14 by $K_{1,\rm edge}$
($50.1\,\kms$) and 2nd by $M_2$
(Table~\ref{tab:astrospectrosb1_population}).
Of the 14 candidates, 7 are GOLD-tier and 3 are SILVER-tier;
this source is the highest-$M_2$ GOLD-tier candidate in the
AstroSpectroSB1 sample.
...
We emphasise that the \texttt{AstroSpectroSB1} solution type
provides a critical advantage over purely astrometric
\texttt{Orbital} solutions: the orbital signal is independently
detected in both photo-centre motion and radial velocities,
dramatically reducing the probability of a spurious detection.
...
\subsection{Binary evolution considerations}
\label{sec:discussion:binary_evolution}

A $\sim$\,$12.5\,\Msun$ BH is expected to form from a zero-age
main-sequence progenitor of $\gtrsim 25$--$30\,\Msun$
\citep{Fryer2012, Belczynski2020}.  The wide orbit
($a \approx 5.7\,$AU, $P = 1352\,$d) requires that mass transfer
during the progenitor's giant phase either did not occur or was
non-conservative.  In the standard isolated binary evolution
channel, common-envelope ejection or stable Roche-lobe overflow
can produce wide BH--star binaries, but the resulting orbital
separations and eccentricities depend sensitively on the mass
ratio, the binding energy of the envelope, and the natal kick.

The moderate eccentricity ($e = 0.532$) is notable: purely
adiabatic mass loss in a circular orbit produces $e_{\rm post}
\leq 0.06$ for typical mass ratios \citep{Blaauw1961}.  The
observed eccentricity therefore implies either a natal kick
($v_{\rm kick} \gtrsim 30\,\kms$) or a dynamical interaction.
This is consistent with the natal-kick distribution inferred
from X-ray binaries \citep{Repetto2012}.
The tidal circularisation timescale at $P = 1352\,$d is much
longer than the Hubble time for a K-giant primary, so the
present eccentricity is effectively frozen.
...
\item \textbf{Parallax uncertainty.}  $\varpi = 0.672 \pm
  ...
  change in $\varpi$ shifts $M_2$ from $\sim$\,7.7 to
  $\sim$\,17.1$\,\Msun$.
...
\item \textbf{NSS covariance.}  The full covariance matrix of the
  ...
  procedure treats $M_1$, $P$, and $\varpi$ as independent.
  Correlation sensitivity tests
  (Section~\ref{sec:methods:correlation}) show this is
  conservative for the most physically motivated correlation
  ($\varpi$--$a_{\rm ang}$).
\item \textbf{$T_{\rm eff}$ chain dependency.}  If the GSP-Phot
  $T_{\rm eff}$ is biased by the high RUWE, the entire
  downstream inference shifts
  (Section~\ref{sec:methods:teff_chain}).  However, $M_2$
  remains above $5\,\Msun$ even for offsets of $\pm 500\,$K.
...
\item \textbf{$M_1$ sensitivity.}  The $M_2/M_1$ ratio of
  ...
  $>$99~per~cent for all plausible $M_1$ values.  The PARSEC
  cross-check (Section~\ref{sec:methods:m1}) confirms the
  adopted $M_1$ is reasonable.
...
This paper presents a single-object characterisation dossier
and follow-up prioritisation assessment.  While the
\texttt{AstroSpectroSB1} solution type provides strong dual-technique
evidence, the companion mass and all scenario exclusions remain
conditional on the Gaia pipeline model.  Independent multi-epoch
radial-velocity monitoring would provide definitive dynamical
confirmation and is strongly recommended.

\section{Full AstroSpectroSB1 BH candidate catalogue}
\label{app:full_catalogue}

Table~\ref{tab:astrospectrosb1_population} lists all 14
\texttt{AstroSpectroSB1} sources with pipeline-reported companion
mass $M_2 > 5\,\Msun$ from the GRAVITAS OmniScan v13 catalogue,
ranked by $M_2$.  This source ranks 2nd by $M_2$ and 1st
among GOLD-tier candidates.

\begin{table*}
\centering
\caption{All 14 \textsc{AstroSpectroSB1} BH candidates from the
GRAVITAS OmniScan catalogue (v13).
The target of this study is highlighted in bold.}
\label{tab:astrospectrosb1_population}
\begin{tabular}{lrrrrrrrl}
\hline
Source ID (approx.) & $G$ & $P$ (d) & $e$ & $M_1$ ($\Msun$) & $M_2$ ($\Msun$) & $\varpi$ (mas) & Tier \\
\hline
3892196$\times 10^{18}$ & 7.40 & 1759.5 & 0.109 & --- & 19.27 & --- & BRONZE \\
\textbf{5870569$\times 10^{18}$} & \textbf{12.28} & \textbf{1352.3} & \textbf{0.532} & \textbf{1.04} & \textbf{12.55} & \textbf{0.672} & \textbf{GOLD} \\
4490794$\times 10^{18}$ & 11.70 & 486.2 & 0.255 & --- & 11.29 & --- & GOLD \\
692539$\times 10^{15}$ & 9.30 & 587.6 & 0.176 & --- & 9.08 & --- & GOLD \\
5296920$\times 10^{18}$ & 11.50 & 1212.7 & 0.198 & --- & 8.45 & --- & BRONZE \\
3286049$\times 10^{18}$ & 8.20 & 872.7 & 0.054 & --- & 6.65 & --- & GOLD \\
5984963$\times 10^{18}$ & 12.90 & 781.5 & 0.341 & --- & 6.58 & --- & GOLD \\
240409$\times 10^{15}$ & 10.00 & 813.4 & 0.042 & --- & 6.52 & --- & GOLD \\
3389049$\times 10^{18}$ & 12.30 & 964.5 & 0.441 & --- & 6.06 & --- & SILVER \\
5618977$\times 10^{18}$ & 9.60 & 436.7 & 0.005 & --- & 5.86 & --- & GOLD \\
5932493$\times 10^{18}$ & 10.30 & 1653.5 & 0.344 & --- & 5.56 & --- & SILVER \\
1986665$\times 10^{18}$ & 8.90 & 1092.4 & 0.569 & --- & 5.37 & --- & GOLD \\
284795$\times 10^{15}$ & 8.00 & 1081.5 & 0.945 & --- & 5.32 & --- & SILVER \\
530854$\times 10^{15}$ & 11.90 & 1729.0 & 0.565 & --- & 5.18 & --- & BRONZE \\
\hline
\end{tabular}
\end{table*}
...
\begin{table}
\centering
\caption{PARSEC-calibrated M1 grid summary.  The grid spans
$[\mathrm{Fe/H}] \in [-0.5,\,+0.3]$ and age $\tau \in [1,\,12]\,$Gyr
at $T_{\rm eff} = 4832\,$K, $\log g = 3.255$.}
\label{tab:parsec_grid}
\begin{tabular}{lcc}
\toprule
Quantity & Value & Note \\
\midrule
Grid range       & $0.67$--$1.83\,\Msun$ & Full $[\mathrm{Fe/H}]$--age grid \\
Grid median      & $0.94\,\Msun$          & --- \\
MC median        & $0.90\,\Msun$          & Broad priors \\
MC 68\% CI       & $[0.66,\,1.30]\,\Msun$ & --- \\
MC 90\% CI       & $[0.53,\,1.69]\,\Msun$ & --- \\
Adopted $M_1$    & $1.04\,\Msun$          & Within central body \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}
\centering
\caption{Atmosphere-corrected and synthetic flux ratios (companion
$12.55\,\Msun$ MS / primary K-giant) using photometric $R = 9.8\,\Rsun$.
All ratios are far above the 5~per~cent detection threshold.}
\label{tab:fluxratios}
\begin{tabular}{lccc}
\toprule
Band & Planck & Atm.-corr. & Synthetic \\
\midrule
$G_{\rm BP}$ & 31.6 & 34.5 & 32.7 \\
$G$          & 15.2 & 16.2 & 15.7 \\
$G_{\rm RP}$ & 8.0  & 8.2  & 8.2  \\
$J$          & 3.3  & 3.3  & 3.4  \\
$H$          & 2.3  & 2.3  & 2.4  \\
$K$          & 1.9  & 1.9  & 1.9  \\
$W1$         & 1.5  & 1.5  & 1.5  \\
...
\begin{table}
\centering
\caption{Correlation sensitivity tests.  $P(M_2 > 5\,\Msun)$
under various inter-parameter correlation assumptions.  Independent
sampling ($\rho = 0$) is conservative for the $\varpi$--$a_{\rm ang}$
pair.}
\label{tab:correlation}
\begin{tabular}{lcccc}
\toprule
Test & $\rho = 0$ & $\rho = 0.3$ & $\rho = 0.5$ & $\rho = 0.7$ \\
\midrule
$\varpi$--$a_{\rm ang}$ & 88\% & 91\% & 94\% & 96\% \\
$P$--$e$               & 91\% & 91\% & 91\% & 91\% \\
\bottomrule
\end{tabular}
\end{table}
